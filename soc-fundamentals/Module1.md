...
If Tier 1 fails, the adversary goes unnoticed. If Tier 2 fails, the incident response is botched. When an active threat is confirmed, Tier 2 analysts are typically authorized to activate containment actions: isolating endpoints, severing network connections, or disabling accounts. If they act too hastily without understanding the scope, they might tip off the attacker, who may deploy ransomware and burn the network down before they can be locked out. If they act too slowly, the data exfiltration completes. If they act incorrectly, they could upset a worker who will certainly tell everyone about security's incompetence.
...
*A note on the Analyst role, and the roles that follow.* Role functionality dictates operational timeframes. Interrupt-driven tasks define the analyst tier. Analysts, Engineers, and Architects operate on increasingly longer focus periods.
...
* **Executive Translator:** The CISO translates technical risk into financial exposure. They secure funding from the Board of Directors, or CIO/CTO, depending on the organization.
...
Analysts continuously monitor the digital enterprise. They ingest telemetry from endpoints, network perimeters, cloud services, and identity platforms. SIEM platforms centralize this data, aiming to provide complete visibility, allowing the Security Operations Center to detect malicious anomalies beyond what any given tool may detect. The tier 3 Analysts will be active here, crafting custom alerts, threat hunting, and otherwise applying human intelligence to combat human intelligence.

Analysts work to establish a baseline of normal environmental behavior. They compare real-time feeds against this baseline, and this continuous observation identifies unauthorized access attempts immediately. Blind spots invite failure, so it is important to ensure that everything is assessed for its importance and logged accordingly. In fact, even the logging needs logging to ensure that logging doesn't fail and go unnoticed!

### Threat Detection and Alert Triage

Security platforms generate automated alerts based on suspicious activity, which Tier 1 analysts triage daily. They isolate true positive threats from baseline noise, investigate the initial vector of compromise, and escalate as needed when Tier 2 assistance is required.

Analysts correlate disjointed logs to build a coherent narrative, as even when everything is poured into a SIEM, it is rarely all properly tuned and baselined. Often, the Analyst will pivot into other security tools to pull additional logs if something is missing. Rarely are things perfect and all fully ingested into the sales term called "Single Pane of Glass". They package this narrative for rapid escalation. Efficient triage filters out false positives. When everything works properly, this process prevents cognitive overload across the broader team.

### Incident Response

The Security Operations Center acts as the first responder during a cyber event. Tier 2 analysts execute predefined playbooks to contain active threats, isolate compromised systems from the production network, and disable compromised user accounts. They must operate with awareness of the threat, escalate to the Tier 3 Analysts or SOC Manager as required, and be mindful that they may need to monitor a bad actor before terminating access, lest the bad actor trigger a fail-safe of their own and cause destruction to an environment. Additionally, during IR, a thought towards possible forensics must be considered, as rebooting a laptop may clear critical information from memory.

Analysts execute immediate containment to preserve business continuity whenever possible. They work to eradicate malicious activity and extract persistent artifacts before coordinating service recovery with systems administrators. The team also documents the complete response lifecycle for post-incident review, a critical step for ensuring future improvement, and to understand what gap was exploited and, in the near future, will hopefully be patched.
...
We'll look over the specialized platforms needed for a successful SOC. These platforms include orchestration systems, endpoint detection agents, and network sensors. Standardized tooling enforces the defensive posture, while proper integration reduces response times and eliminates coverage gaps.

## Proactive Operations

Proactive operations require advanced operational maturity. Analysts actively hunt for hidden adversaries before systems trigger alarms, scan the environment for unpatched vulnerabilities, and apply threat intelligence to predict attacks. This approach neutralizes threats before they cause operational impact, as security is always less expensive when applied earlier in a process.

The CISO authors the overall security policy and sets the standard after consulting with the rest of the business. This policy sets cyber hygiene standards, which the SOC monitors for policy deviations. The exact scope of proactive security responsibility varies by organization, but I include it here for completeness.
...
The SOC manages the central security data repository. Security Engineers and SOC Management ensure log retention meets legal and regulatory requirements. Because they work within these tools daily, SOC Analysts are typically the ones who generate the reports needed to prove organizational compliance during external audits. To manage costs, logs are often stored in a tiered architecture since SIEM platforms usually charge based on ingestion volume. Bulk data may be routed to cheap, relatively inaccessible cold storage, while an observability pipeline such as Cribl processes the bulk logs and sends only the interesting events to the SIEM. Compliance often is a matter of whether you keep logs and not whether you can easily access them. 

### Threat Hunting

Passive monitoring inherently leaves blind spots. Sophisticated adversaries often evade automated alerts by utilizing zero-day exploits or "living off the land" techniques that blend in with normal network traffic. To counter this, advanced analysts proactively search the environment for hidden threats.

Unlike reactive triage, threat hunting is fundamentally hypothesis-driven. Hunters assume a breach has already occurred and query the data to prove or disprove a specific theory, such as searching for unusual PowerShell activity during off-hours.

Because effective hunting requires a deep understanding of normal network baselines, organizations typically assign this duty to experienced analysts. However, any analyst with available time should practice this skill. Junior team members can build this muscle by translating threat intelligence into search queries, mapping past alerts to frameworks like MITRE ATT&CK, or independently exploring raw log data.

### Digital Forensics

Digital forensics is a highly specialized discipline that rarely operates directly within a standard SOC. The process involves the rigorous preservation, extraction, and analysis of digital evidence, which requires strict chain of custody protocols and specialized legal knowledge. Because of the deep expertise and expensive toolsets required, most organizations do not maintain a dedicated internal forensics team. Even midsize SOCs and MSSPs typically outsource this function to larger, dedicated security firms. These external experts are usually engaged through a proactive retainer and provide forensics bundled as part of a comprehensive Incident Response package to handle major breaches from start to finish.
